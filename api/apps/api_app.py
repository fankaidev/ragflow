...
def token_list():
    try:
        ...
        objs = APITokenService.query(
            tenant_id=tenants[0].tenant_id, dialog_id=request.args["dialog_id"]
        )
...
def stats():
    try:
        ...
        objs = API4ConversationService.stats(
            ...
            request.args.get(
                "from_date",
                (datetime.now() - timedelta(days=7)).strftime("%Y-%m-%d 24:00:00"),
            ),
...
def completion():
    ...
    try:
        ...
        def stream():
            ...
            try:
                for ans in chat(dia, msg, True, **req):
                    ...
                    yield "data:" + json.dumps(
                        {"retcode": 0, "retmsg": "", "data": ans}, ensure_ascii=False
                    ) + "\n\n"
            ...
            yield "data:" + json.dumps(
                {"retcode": 0, "retmsg": "", "data": True}, ensure_ascii=False
            ) + "\n\n"
...
def list_kb_docs():
    ...
    try:
        print("kb_id", kb_id, page_number, items_per_page, orderby, desc, keywords)
